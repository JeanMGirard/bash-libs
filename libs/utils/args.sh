...
#>>
#>> * `args::parse [-S|--FULL)VARIABLE;FLAGS[,COND]...]... -- [ARGUMENTS]...`
#>
#> Where:
#>
#> * `-S`       - short option name.
...
#> Options `--help` and `--man` are calling `args::help()` function with `2` or `1` as
...
args::parse() {
  ...
  PARSER="$(args::generate_parser "${OPTION_DESCRIPTIONS[@]:+${OPTION_DESCRIPTIONS[@]}}")" || return 1
  eval "$PARSER" || return 1
  args::parse_options "${@:+$@}" || return $?
}

#>>
#>> * `args::generate_parser OPTIONS_DESCRIPTIONS` - generate parser for options.
#> Will create function `args::parse_options()`, which can be used to parse arguments.
#> Use `declare -fp args::parse_options` to show generated source.
  ...
  while [ $# -gt 0 ]; do
    ...
    case "$OPTION_DESCRIPTION" in
      ...
      *) error "Incorrect syntax of option: \"$OPTION_DESCRIPTION\". Option syntax: -S|--FULL)VARIABLE;TYPE[,CHECK]... . Example: '-f|--foo)FOO;String,Required'."
        __log__DEBUG=yes; stacktrace; return 1; ;;
    esac
    ...
    case "$OPTION_TYPE" in
     y|yes|Y|Yes) # Set variable to "yes", no arguments
...
#>>
#>> * `args::help LEVEL` - display embeded documentation.
